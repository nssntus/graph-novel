...
**状态**：批次 0–6 的 Mock 闭环完成；真实 API smoke test 待费用授权
**制定日期**：2026-07-27  
...
- Python 3.9.6。
- 原始回归基线为 9 项；当前 Mock 集成测试为 24 项。
...
- [x] 记录每次运行的节点、开始/结束时间、耗时、路由决定和错误摘要。
- [x] 为 Prompt/响应保留可控、脱敏、可选的调试记录。
- [x] 修正创建表单、审批按钮、驳回重写和导出中的中英文不一致。
- [x] 增加完整 Web 流程测试和必要的浏览器验证。
- [x] 核对字数、章节数、导出文件名和全局终审前置条件。
- [x] 核心执行稳定后，再评估是否引入显式图框架（当前无迁移收益证据，保持轻量引擎）。
...
### 2026-07-27：第五轮可观察性、统一导出与 Web 体验

- 完成批次 6；State 升级到 v5，以 `execution_events` 持久化节点开始/结束、耗时、状态、路由目标和选择原因。
- Engine 继续作为路由唯一负责人；Foundation、自动重写、章节审批和全局终审的关键边均有可追溯事件。
- 新增可选 `GRAPH_NOVEL_LLM_DEBUG_FILE` JSONL 调试日志；默认关闭，内容截断并脱敏，日志写入失败不影响模型主调用。
- 新增 `graph_novel/exporting.py`，Engine、Web 和 CLI 统一以“已批准且有正文”为完成/导出标准；完整 Markdown 只包含批准稿。
- Web API 暴露最近 100 条执行事件及中文节点/审批点标签；页面中的 Foundation、Phase、Gate 和角色阶段等用户可见术语已统一为中文。
- 新增 5 章 Mock Web 端到端测试，覆盖创建、Foundation Gate、五个 Chapter Gate、全局终审和下载。
- Mock 集成测试为 24 项：`python3 tests/test_integration.py`，24/24 通过；Python 3.9 编译检查与 `git diff --check` 通过。
- 本地浏览器验证首页、创建页、基础设定页、项目面板和章节页通过；批准进度为 0/5，页面控制台无错误。
- 未读取 `.env`，未调用真实 DeepSeek API，未产生 API 费用。
- 当前结论：现有轻量 Engine 已能清晰表达 Node、State、Edge、Gate、Loop 与 Checkpoint；尚无引入 LangGraph 等框架的必要证据。
- 已知边界：项目运行锁仍是单进程互斥；执行事件保存在项目 JSON 中，若未来扩展到多实例或超长项目，再评估持久任务队列与独立事件存储。
- 下一入口：只有在用户明确授权 API 费用后执行最小真实 DeepSeek smoke test；否则按实际使用反馈进入下一批。
