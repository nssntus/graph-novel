...
- 当前目录不是可识别的 Git 工作树，实施前缺少版本回滚基线。
- `GraphNovelEngine` 已包含三阶段、审批和重写循环，但状态转换尚未完整持久化。
...
- [ ] 保留 CLI 交互，但让 CLI 通过同一组 Engine 动作驱动，不复制图规则。
- [ ] 节点失败时停止下游调度，并记录失败节点和可恢复位置。
...
- [ ] 章节规划更新既有 `Chapter`，不无条件替换整个对象。
- [ ] 自动重写时把上一轮 `consistency_report.issues` 传给规划和写作节点。
- [ ] 人工驳回时把 `human_feedback` 传给下一轮规划和写作节点。
- [ ] 写作节点先保存候选 META，不立即提交人物弧线和伏笔副作用。
- [ ] 仅在章节批准时幂等提交弧线、伏笔和章节钩子等全局副作用。
- [ ] 明确自动重写次数和人工重写次数的记录方式。
...
- [ ] 为运行操作返回 `202 Accepted` 和任务状态，而不是长时间 `join`。
- [ ] 增加项目级运行锁，拒绝同一项目的冲突任务。
- [ ] API 区分 `idle`、`running`、`awaiting_approval`、`failed`、`completed`。
- [ ] 前端轮询项目/任务状态，展示当前节点、失败原因和可执行动作。
- [ ] 服务重启时依据 checkpoint 恢复为安全状态；不把丢失的线程视为仍在运行。
- [ ] 清理完成的内存任务和审批事件，避免长期泄漏。
