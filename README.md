...
> **让 AI 不只是“继续写”，而是真正记得故事为什么走到这里。**

GraphNovel（番茄小说工坊）是一套面向中文网络小说的 Graph Engineering 创作系统。它把世界观、人物、大纲、章节规划、正文、润色、审查和人工决策连接成一张可运行、可追踪、可恢复的创作图，让长篇写作从一次次碰运气的生成，变成有人把关、有状态延续、有逻辑约束的持续创作流程。

无论你是在构思第一本长篇，还是已经被“角色突然知道不该知道的事”“上一章埋下的线索下一章就消失”反复折磨，GraphNovel 都试图解决同一个核心问题：**让故事真正承接，而不只是字数继续增长。**

[观看演示](#界面演示) · [快速开始](#快速开始) · [了解创作图](#graph-engineering) · [查看主要能力](#主要能力)

项目提供开箱即用的本地 Web 界面和 CLI，通过 OpenAI SDK 调用 DeepSeek 兼容接口。创作数据默认保存在本机，关键节点由你决定是否继续。

## 界面演示

[观看 74 秒产品演示视频](docs/assets/graphnovel-demo.mp4)

### 项目面板

![GraphNovel 项目面板](docs/assets/dashboard.png)

### 章节阅读与人工审批

![GraphNovel 章节阅读](docs/assets/chapter-review.png)

### 全局终审

![GraphNovel 全局终审](docs/assets/global-review.png)

## 为什么是 GraphNovel

普通 AI 写作工具擅长生成一段“看起来像小说”的文字；长篇创作真正困难的，却是让数十章、数百章之间仍然保持因果、认知和人物状态的一致。GraphNovel 从这个难点出发设计：

- **不是 Prompt 接龙，而是可执行的创作图**：每个 Agent 只负责自己的节点，引擎根据结果选择下一条边。
- **不是把所有旧正文塞回上下文**：系统保留上一章的直接承接，并用有界 Narrative State 提供更早的关键事实。
- **不是发现问题就整章推倒重来**：规划、正文、润色和事实契约拥有独立的重写路径与次数上限。
- **不是让模型替你拍板**：Foundation 和每一章都停在持久化审批门前，是否通过、从哪里修改，由创作者决定。

GraphNovel 想做的不是一台“一键爆款小说机”，而是一套更可靠的 AI 长篇创作工作台：模型负责生成与检查，图负责秩序，你负责方向和最终判断。
...
- 希望借助 AI 提高产量，但不愿牺牲剧情连续性的网文作者。
- 需要长期维护世界观、人物认知、伏笔和资源变化的长篇创作者。
- 想研究 Graph Engineering、Agent 编排、持久化 Gate 与受控 Loop 的开发者。
- 正在搭建 AI 内容生产流程，希望保留人工审批权和完整执行记录的团队。
...
仓库内置《循环之外》演示项目，不调用模型即可查看完整的世界观、人物、五章正文、Narrative State、受控重写记录和终审报告：

```bash
GRAPH_NOVEL_DIR=examples python3 web_server.py
```

打开 `http://127.0.0.1:5500`，选择《循环之外》。示例内容为项目原创演示数据，不代表真实模型输出质量。详细说明见 [`examples/README.md`](examples/README.md)。
